...
```
和全局变量类似，它们在程序启动的时候获得内存，但不进行初始化。在第一次执行声明语句时，进行初始化；此后再次执行声明语句时什么也不做，不会再次初始化。比如这里 main 函数调用了三次 `f`，但因为初始化只执行一次，所以输出为 `43 44 45`。静态局部变量的存储空间不随着函数的调用和返回而变动，所以不受函数空间大小的限制。
