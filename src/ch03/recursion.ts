...
// Fig. 1
export function fig1(element: HTMLElement) {
  element.setAttribute("style", "min-width: 550px");
  const fig = Raphael(element, 420, 380);
  const ram = createRAM(fig, 20, 50, 380, 300);
  const main = createFunction(fig, "main", 40, 100, 100, 200);
  const prime = createFunction(fig, "isPrime", 160, 100, 100, 200);
  const sqrt = createFunction(fig, "sqrt", 280, 100, 100, 200);
  const mainCode = [
    "int a;          ",
    "cin >> a;       ",
    "if (isPrime(a)) ",
...
export function fig2(element: HTMLElement, textElement: HTMLElement) {
  element.setAttribute("style", "min-width: 550px");
  const fig = Raphael(element, 550, 450);
  const ram = createRAM(fig, 20, 50, 500, 300);
  const main = createFunction(fig, "main", 30, 100, 80, 200);
  const facts = [
  ...
  const cout = new StatementSnippet(main, 10, 120, 60, 30, "cout...");
  const mainCall = new StatementSnippet(main, 10, 50, 60, 30, "fact(4);");
  const ns = [];
  const returns = [];
  const calls = [];
  ...
  facts.forEach((func, index) => {
    ns.push(
      new VariableSnippet(func, 10, 20, 60, 30, "n").setValue(
        (4 - index).toString()
      )
    );
    if (index < 3) {
      calls.push(
        new StatementSnippet(func, 10, 90, 60, 30, `fact(${3 - index});`)
      );
    }
    returns.push(
      new StatementSnippet(
        func,
        10,
        150,
        60,
  ...
  const actions = [
    () => {
      ...
      ns.forEach((e) => e.hide()),
        calls.forEach((e) => e.hide()),
        returns.forEach((e) => e.hide());
    },
    () => {
      ram.show();
    },
  ...
  new Button(fig, textElement, 20, 370, texts, actions).insert();
}
