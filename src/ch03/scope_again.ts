...
export function fig1(element: HTMLElement, textElement: HTMLElement) {
  ...
  const main = createFunction(fig, "main", 40, 200, 120, 100);
  const change = createFunction(fig, "change", 180, 200, 120, 100);
  const call = new StatementSnippet(main, 10, 20, 100, 30, "change(a,b);");
  const cout = new StatementSnippet(main, 10, 60, 100, 30, "cout<<a<<b;");
  const assign = new StatementSnippet(change, 10, 20, 100, 30, "a=30,b=50;");
  const a = new VariableSnippet(ram, 100, 20, 100, 30, "a");
  const b = new VariableSnippet(ram, 100, 60, 100, 30, "b");
  const info = [
    "",
    "计算机首先找到一片完整的内存。",
